...
export default function FAQ({ items = DEFAULT_FAQS, embedded = false }: FAQProps) {
  ...
  const content = (
    <>
      ...
      <div className="space-y-4">
        {items.map((faq, index) => (
          <div key={faq.question} className="bg-card rounded-lg overflow-hidden border border-border">
            <button
  ...
  if (embedded) {
    return (
      <div id="faq" className="mt-20 max-w-4xl mx-auto">
        {content}
      </div>
    );
  }
